...
export function Header({ onOpenSidebar }: HeaderProps) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      // Try canvas-based PDF first
      await generatePDF('pdf-content', currentScenario);
    } catch (error) {
      // Fall back to simple PDF if canvas fails
      console.warn('Canvas PDF failed, using simple PDF:', error);
      generateSimplePDF(currentScenario);
    } finally {
